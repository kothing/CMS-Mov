						...
						<div class="stui-pannel-box clearfix">
							...
							<form method="post" class="messages">
								<div class="col-pd clearfix">
									<ul class="gbook-form">
										<li>
											<input class="form-control" type="text" placeholder="匿名" id="input1" onblur="jieshou()" name="userid">
										</li>
										...
										<li><input id="verifycode" class="form-control" name="verifycode" type="text" size="10" placeholder="验证码" style="width: 80px; display: inline-block; margin-right: 10px;" class="text" tabindex="3" />
											<img style="height: 28px; cursor:pointer;" src="../system/verifycode.php" onclick="javascript:this.src='../system/verifycode.php?'+Math.random()" style="cursor:pointer;" title="看不清？点击更换" align="absmiddle" />
										</li>
					...
					while ($row = mysqli_fetch_array($result)) { ?>
						...
						<div class="stui-pannel stui-pannel-bg clearfix">
							<div class="stui-pannel-box clearfix">
								<div class="col-pd clearfix">
									<ul>
										<li class="topwords"><strong><?php echo $row['userid'] ?></strong><span class="text-red pull-right">第<?php echo $row['id'] ?>楼</span></li>
										<li class="top-line" style="margin-top: 10px; padding: 10px 0;"><?php echo $row['content'] ?><br />
											<span style="color: red">
												<?php if (0 < strlen($zhStr)) {
													echo '管理员回复：';
													echo $row['Reply'];
												} ?>
											</span>
										</li><span class="font-12 text-muted">发表于: <?php echo $row['time'] ?></span>
									</ul>
								</div>
							</div>
						</div>
