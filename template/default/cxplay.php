...
<body class="video-play-page">
		...
		<div class="row">
			<div class="col-lg-wide-75 col-xs-1">
				<div class="stui-player__item myui-player__item clearfix" style="background-color: #fff;">
					...
					<?php
					if (empty($data['data'][0]['vod_year'])) {
						$nian = $data['data'][0]['d_year'];
					} else {
						$nian = $data['data'][0]['vod_year'];
					}
					if (!empty($data['data'][0]['vod_director'])) {
						$daoyan = '导演：' . $data['data'][0]['vod_director'];
					}
					if (!empty($data['data'][0]['vod_actor'])) {
						$zhuyan = '主演：' . $data['data'][0]['vod_actor'];
					}
					if (!empty($data['data'][0]['vod_continu'])) {
						$zuyuangx = '状态：' . $data['data'][0]['vod_continu'];
					}
					?>
					<div class="stui-player__detail detail" id="dianshijuid">
						...
						<?php if (!empty($data['data'][0]['vod_continu'])) {
							echo '<span class="split-line"></span>';
						} ?>
									...
									foreach ($d2 as $k2 => $v2) {
										foreach ($v2 as $cc2) {
											...
											if (strpos($u2[1], 'share')) {
												$urls2 = $u2[1];
												$title2 = $u2[0];
												echo '<li class="active"><a href="' . $urls2 . '" class="btn-play-source" title="' . $strname . ' ' . $title2 . '">' . $title2 . '</a></li>';
											}
										}
									}
								}
	...
	<script type="text/javascript">
		var vod_name = '<?php echo $data['data'][0]['vod_name']; ?>',
